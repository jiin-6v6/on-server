module.exports = {
    basic: function (title, login, nav, content) {
        ...
        table += `</tbody></table></div>`;
        return table;
    }, post_write_update: function (request, response, postId, results) {       // board.js
        var notice = '';
        ...
        return content;
    }, comment_list_update: function (request, results, boardId, postId, commentId) {      // board.js
        ...
        for (i = 0; i < results.length; i++) {
            ...
            var table_comment_writer = `<th style = 'width:70%; text-align:left; vertical-align:middle;'>${comment_writer}</th>`;
            ...
            var button = `<th style = 'width:5%; text-align:center; vertical-align:middle;'></th>
            <th style = 'width:10%; text-align:center; vertical-align:middle;'>${comment_update}</th>
            <th style = 'width:10%; text-align:center; vertical-align:middle;'>${comment_delete}</th>
            <th style = 'width:5%; text-align:center; vertical-align:middle;'></th>`;

            // 댓글 내용
            var table_comment_content = `<th style = 'width:70%; text-align:left; vertical-align:middle;'>${results[i].comment_content.replace(/\r\n/gi, "<br>")}</th>`;
            if (results[i].parent_id !== 0) {   // 대댓글인 경우
                table_comment_wirter = `<th style = 'width:10%; text-align:center;'>↳</th>
                    <th style = 'width:60%; text-align:left; vertical-align:middle;'>${comment_writer}</th>`;
                table_comment_content = `<th style = 'width:10%; text-align:center;'></th>
                    <th style = 'width:60%; text-align:left; vertical-align:middle;'>${results[i].comment_content}</th>`;
            }
            if (commentId && (results[i].id === commentId)) {   // comment update mode
                table_comment_content = `<th colspan='5' style = 'width:100%;text-align:left; vertical-align:middle;'>
            ...
            var index = (results[i].parent_id === 0) ? 1 : array.findIndex(element => element.includes(`div id="${results[i].parent_id}"`));
            array.splice(index, 0, `<tr id="${results[i].id}_info">
                            ${table_comment_writer}
                            <th colspan='4' style = 'width:30%; text-align:center; vertical-align:middle;'>${results[i].time}${isUpdate}</th>
                        </tr>
        ...
        table += `</tbody></table></div>`;
        return table;
    }, postlist_mycomment: function (results, this_page) {
        var table = `<div class='table-responsive'>
        <table class='table table-nomargin'>
            <thead>
            <tr><th style='min-width:40px' class='mobile-hide'><center>번호</center></th><th style='width:60%; padding-left:12px !important;'><center>제목</center></th><th style='min-width:80px;text-align:center;'>수정여부</th><th style='width:15%;text-align:center;'>날짜</th></tr>
        ...
        list += `</ul></div></div>`;
        return list;
    }, pagination_mycomment: function (this_page, total_page) {
        var list = `<div class="text-center">
        <ul class="pagination">`;
        if (total_page <= 5) {
            list += `<li class="disabled"><a href=""><span class="glyphicon glyphicon-chevron-left"></span></a></li>`;
