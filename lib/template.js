module.exports = {
    basic:function(title, login, nav, content){
        ...
        for(var i=0; i<results.length; i++){
            ...
            <td class='mobile-hide' style = 'font-size : 11px;'><center>`
            table += results[i].id;
            table +=`</center></td>
            <td class='title title-align'>
                <span class="">`;
            table += results[i].post_title;
            table += `</span><td style='text-align:center;'>`;
            table += results[i].post_writer;
            table += `</td><td style='text-align:center;'>`;
            table += results[i].time;
            table += `</td></tr></a>`;
        }
