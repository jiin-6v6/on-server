module.exports = {
    basic: function (title, login, nav, content) {
        return `
            ...
            <body>
                <header>
                    ...
                    <div class="container"><a href="/">다미부기</a></div>
                </header>
        ...
        <tbody id="file_table_0">
                <tr>
                    <td style = 'width:80%; text-align:left;'><input type='file' name='file1'></td>
                    <td style = 'width:10%; text-align:center;'><input type='button' value='확인' onclick='file_write(1);'></td>
                    <td style = 'width:10%; text-align:center;'><input type='button' value='삭제'></td>
                </tr>
                <tr>
                    <td style = 'width:80%; text-align:left;'><input type='file' name='file2'></td>
                    <td style = 'width:10%; text-align:center;'><input type='button' value='확인' onclick='file_write(2);'></td>
                    <td style = 'width:10%; text-align:center;'><input type='button' value='삭제'></td>
                </tr>
                <tr>
                    <td style = 'width:80%; text-align:left;'><input type='file' name='file3'></td>
        ...
        if (!results) {  // update가 아닌 경우 results===null
            var content = `
                ...
                function more_file(){
                    var zero = document.getElementById("file_table_0");
                    var cnt = document.getElementById("fileCnt").value*1;

                    var str = "";

                    if(cnt === 12){
                        alert("파일은 최대 12개까지 업로드 가능합니다.");
                        return false;
                    }

                    str = "<tr>";
                    str += "<td style = 'width:80%; text-align:left;'><input type='file' name='file" + (cnt+1) + "'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='확인' onclick='file_write(" + (cnt+1) + ");'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='삭제'></td>";
                    str += "</tr><tr>";
                    str += "<td style = 'width:80%; text-align:left;'><input type='file' name='file" + (cnt+2) + "'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='확인' onclick='file_write(" + (cnt+2) + ");'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='삭제'></td>";
                    str += "</tr><tr>";
                    str += "<td style = 'width:80%; text-align:left;'><input type='file' name='file" + (cnt+3) + "'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='확인' onclick='file_write(" + (cnt+3) + ");'></td>";
                    str += "<td style = 'width:10%; text-align:center;'><input type='button' value='삭제'></td>";
                    str += "</tr>";
                    cnt += 3;
                ...
                function file_write(n){
                    ...
                    if(textValue.include()){        // 이미 삽입된 경우 지인아 해라
                        alert("이미 삽입된 사진입니다.");
                        return false;
                    }
        ...
        if (results && postId) {
            ...
            <div id="content">
                <form action="/board/update_process/${postId}" method="POST">
                    ...
                    <br><br>
                    <input type="button" onclick="temp();" value="추가하기">
                    <input multiple="multiple" type="file" name="post_file[]" >
                    <br>
            ...
            <script>
                ...
                function temp(){
                    ...
                    var beforeText = textValue.substring(0, selectPos);     // 기존텍스트 ~ 커서시작점 까지의 문자
                    var afterText = textValue.substring(textArea.selectionEnd, txtValue.length);   // 커서끝지점 ~ 기존텍스트 까지의 문자
                    var addText = "hello, world!" // 추가 입력 할 텍스트
        ...
        var after_image = post_content.replace('(사진1)', '<img src="/uploads/1583832633714_샤이니 everybody(color).jpg"></img>');
        ...
        <li style="position:relative;">
                ...
                <button class="regist_btn_a" style="font-size: 10px;" onclick="send_mail();return false;">인증번호보내기</button>
                ...
                <button class="regist_btn_a" onclick="check_num();return false;">확인</button>
                ...
                <script>
                    function check_num(){
                        ...
                        if("${post.user_email}" === checkingEmail){
